  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 font-mono font-bold uppercase tracking-widest border ${
        status === 'active' 
          ...
          : (status === 'draft' || status === 'pending_annulment')
    ...
    >
      <span className={`w-1 h-1 rounded-full animate-pulse ${
        status === 'active' ? 'bg-emerald-400' : status === 'inactive' ? 'bg-slate-400' : (status === 'draft' || status === 'pending_annulment') ? 'bg-amber-400' : 'bg-red-400'
      }`} />
